...
where f.forma_pagamento = 'plano de saúde'; 
-- nota: a consulta acima busca por faturamento via plano. 
-- caso queira filtrar pelo nome do plano cadastrado:
select p.nome, p.cpf 
from paciente p
where p.cpf in (
    select a.cpf_paciente 
    from atendimento a 
    join faturamento f on a.id_atendimento = f.id_atendimento
    where f.forma_pagamento = 'unimed' or f.forma_pagamento = 'plano de saúde'
);

-- exames pendentes
-- quais exames ainda não têm resultado (data_resultado is null) e foram solicitados no mês atual?
select tipo_exame, data_pedido, cpf_paciente 
from exame 
