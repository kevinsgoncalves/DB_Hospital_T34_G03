...
WHERE a.data_atendimento >= date_trunc('month', current_date) - interval '1 month'
AND a.data_atendimento < date_trunc('month', current_date)
GROUP BY m.nome;
...
SELECT p.nome_plano, SUM(f.valor) AS total_faturado
FROM plano_saude p
JOIN credenciamento c ON p.nome_plano = c.nome_plano
JOIN atendimento a ON c.cnpj_hospital = a.crm_medico
JOIN faturamento f ON a.id_atendimento = f.id_atendimento
WHERE f.data_emissao >= '2026-01-01' AND f.data_emissao < '2027-01-01'
